...
def apply_external_force_torque(
    ...
    # print("ASSET CFG BODY IDS", asset_cfg.body_ids, "IS THE BODY IDS")
...
def apply_external_force_torque_preset(
        env: ManagerBasedEnv,
        env_ids: torch.Tensor,
        force_range: tuple[float,float],
        torque_range: tuple[float, float],
        preset_force: list[float],
        preset_torque: list[float],
        link_name: str = "left_elbow_link",
        asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
):
    """Randomize the external forces and torques applied to the bodies.
    ...
    # print(body_id, "IS THE BODY ID")
    print(asset_cfg.body_ids, "IS THE BODY IDS")

    global_body_id = asset.body_names.index(link_name)
    if isinstance(asset_cfg.body_ids, list):
        # asset_cfg.body_ids is a list of global indices (e.g., [18])
        try:
            local_body_id = asset_cfg.body_ids.index(global_body_id)
        except ValueError:
            raise ValueError(f"link_name '{link_name}' not found in asset_cfg.body_ids {asset_cfg.body_ids}")
    else:
        # asset_cfg.body_ids is slice(None) or similar, so local == global
        local_body_id = global_body_id
    ...
    forces = math_utils.sample_uniform(*force_range, size, asset.device)
    torques = math_utils.sample_uniform(*torque_range, size, asset.device)
    print("FORCES", forces.shape, "TORQUES", torques.shape)
    # Set the force/torque only for the target link
    forces[:, local_body_id, :] = torch.tensor(preset_force, device=asset.device)
    torques[:, local_body_id, :] = torch.tensor(preset_torque, device=asset.device)
